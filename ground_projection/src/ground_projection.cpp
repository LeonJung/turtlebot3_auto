...
#include "turtlebot3_auto_msgs/Pixel.h"
#include "turtlebot3_auto_msgs/Vector2D.h"
#include "turtlebot3_auto_msgs/Segment.h"
#include "turtlebot3_auto_msgs/SegmentList.h"
...
public:
  ...
  : nh_("~")
  {
    ...
    h_file_ = ros::package::getPath("ground_projection") + "/config/" + config_name_ + "/calibration/camera_extrinsic/" + config_file_name_ + ".yaml";
    std::ifstream fin(h_file_.c_str());
    if (!fin.good())
    {
      ROS_WARN_STREAM("Can't find homography file: " << h_file_ << " Using default calibration instead.");
      h_file_ = ros::package::getPath("ground_projection") + "/config/" + config_name_ + "/calibration/camera_extrinsic/default.yaml";
    ...
    // Publisher
    pub_lineseglist_ = nh_.advertise<turtlebot3_auto_msgs::SegmentList>("lineseglist_out", 1);
...
private:
  turtlebot3_auto_msgs::Pixel vector2pixel(const turtlebot3_auto_msgs::Vector2D& vec)
  {
    float w = static_cast<float>(camera_info_->width);
    float h = static_cast<float>(camera_info_->height);

    turtlebot3_auto_msgs::Pixel pixel;
  ...
  turtlebot3_auto_msgs::Vector2D pixel2vector(const turtlebot3_auto_msgs::Pixel pixel)
  {
    float w = static_cast<float>(camera_info_->width);
    float h = static_cast<float>(camera_info_->height);

    turtlebot3_auto_msgs::Vector2D vec;
  ...
  void image2ground(const turtlebot3_auto_msgs::Vector2D& vec, geometry_msgs::Point& point)
  {
    turtlebot3_auto_msgs::Pixel pixel = vector2pixel(vec);
    image2ground(pixel, point);
  }

  void ground2image(const geometry_msgs::Point& point, turtlebot3_auto_msgs::Vector2D& vec)
  {
    turtlebot3_auto_msgs::Pixel pixel;
  ...
  void image2ground(const turtlebot3_auto_msgs::Pixel& pixel, geometry_msgs::Point& point)
  {
  ...
  void ground2image(const geometry_msgs::Point& point, turtlebot3_auto_msgs::Pixel& pixel)
  {
  ...
  void lineseglist_cb(const turtlebot3_auto_msgs::SegmentList::ConstPtr& msg)
  {
    turtlebot3_auto_msgs::SegmentList msg_new = *msg;
  ...
  bool estimate_homography_cb(std_srvs::Empty::Request& request, std_srvs::Empty::Response& response)
  {
    ...
    // Write to yaml file
    std::string wrtie_file_path = ros::package::getPath("ground_projection") + "/config/" + config_name_ + "/calibration/camera_extrinsic" + ros::this_node::getNamespace() + ".yaml";
